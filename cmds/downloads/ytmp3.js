...
❒ Enlace › ${url}`
  ...
  const fileName = audio.name || `${sanitizeFileName(title)}.mp3`

  const sections = [
    {
      title: "Opciones de Audio",
      rows: [
        { title: "Transmitir en Llamada", rowId: `${usedPrefix}voipcall ${audio.url}`, description: `Iniciar llamada de voz usando este audio` }
      ]
    }
  ]

  const listMessage = {
    text: `🎵 *${title}* se ha procesado con éxito.\n\nElige una opción abajo si deseas usarlo en una llamada de voz, o espera el envío del archivo directo.`,
    footer: "Downloads",
    title: "Acciones Disponibles",
    buttonText: "Seleccionar Acción",
    sections
  }

  await sock.sendMessage(msg.chat, listMessage, { quoted: msg })
...
export const voipCmd = {
command: ['voipcall'],
category: 'downloads',
description: 'Llamar usando un enlace de audio remoto.',
run: async ({ msg, sock, args }) => {
  if (!args[0]) return

  const audioUrl = args[0]
  const tempFilePath = path.join(`./temp_voip_${msg.key.id}.mp3`)
  ...
  try {
    ...
    const call = await client.call(msg.sender.split('@')[0], {
