...
❒ Enlace › ${url}`
      ...
      if (shouldCall) {
        ...
        if (!audioResponse.ok) {
          throw new Error(`No se pudo descargar el audio: HTTP ${audioResponse.status}`)
        }

        const audioBuffer = Buffer.from(await audioResponse.arrayBuffer())
        const tempFilePath = `./temp_audio_${randomUUID()}.mp3`
        ...
        try {
          ...
          const client = new VoipClient({ sock, baileys })
...
const isYTUrl = (url = '') =>
  /^(https?:\/\/)?((www|m|music)\.)?(youtube\.com|youtu\.be)\/.+$/i.test(String(url).trim())
