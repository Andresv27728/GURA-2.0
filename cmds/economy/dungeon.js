  ...
  run: async ({ msg, sock, usedPrefix }) => {
    ...
    try {
      ...
      if (user.stamina < staminaConsumed) {
        return sendInteractive(`ꕥ No tienes suficiente stamina para volver asaltar la mazmorra.\n> Usa *${usedPrefix}heal* para curarte.`);
      }
      ...
      if (user.health < 5) {
        return sendInteractive(`ꕥ No tienes suficiente salud para volver a la *mazmorra*.\n> Usa *${usedPrefix}heal* para curarte.`);
      }
      if (Date.now() < user.lastdungeon) {
        const restante = user.lastdungeon - Date.now();
        return sendInteractive(`ꕥ Debes esperar *${msToTime(restante)}* antes de volver a la mazmorra.`);
      }
      ...
      if (rand < 0.4) {
        ...
        const successMessages = [
          ...
          `Descifraste los símbolos rúnicos y obtuviste recompensas ocultas.`,
          `Encuentras al sabio de la mazmorra, quien te premia por tu sabiduría.`,
          `El espíritu de la reina ancestral te bendice con una gema de poder.`,
          `Superas la prueba de los espejos oscuros y recibes un artefacto único.`,
          `Derrotas a un gólem de obsidiana y desbloqueas un acceso secreto.`,
          `Salvas a un grupo de exploradores perdidos y ellos te recompensan.`,
          `Consigues abrir la puerta del juicio y extraes un orbe milenario.`,
          `Triunfas sobre un demonio ilusorio que custodiaba el sello perdido.`,
          `Purificas el altar corrompido y recibes una bendición ancestral.`
        ];
        message = `🏆 ${pickRandom(successMessages)}\n> 📈 Ganaste *¥${cantidad.toLocaleString()} ${currency}*`;
      } else if (rand < 0.7) {
        if (usingWeapon) {
          user.weapons.hacha.durability -= durabilityConsumed;
          if (user.weapons.hacha.durability <= 10) delete user.weapons.hacha;
        ...
        const failMessages = [
          ...
          `Un basilisco te sorprende en la cámara oculta, huyes herido.`,
          `Una criatura informe te roba parte de tu botín en la oscuridad.`,
          `Fracasas al invocar un portal y quedas atrapado entre dimensiones.`,
          `Pierdes el control de una reliquia y provocas tu propia caída.`,
          `Un grupo de espectros te rodea y te obliga a soltar tu tesoro.`,
          `El demonio de las sombras te derrota y escapas con pérdidas.`
        ];
      ...
      return sendInteractive(`「✿」 ${message}`);
    } catch (e) {
