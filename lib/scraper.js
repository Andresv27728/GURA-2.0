...
// instagram-downloader is an optional dependency; import it dynamically when needed
let instagramdl = null;

async function ensureInstagramDownloader() {
  if (instagramdl) return instagramdl;
  try {
    const mod = await import('@mrnima/instagram-downloader');
    instagramdl = mod?.instagramdl || mod?.default?.instagramdl || mod?.default || mod;
    return instagramdl;
  } catch (e) {
    // leave instagramdl null and let caller handle the absence
    return null;
  }
}
...
async function facebookDl(url) {
	try {
		const { baseURL, apiKey } = config.api.yosoyyo;
		const api = `${baseURL}/facebook?url=${encodeURIComponent(url)}&apiKey=${apiKey}`;
		const res = await axios.get(api);
		const json = res.data;

		if ((json.status === 200 || json.status === true) && json.result && json.result.media) {
			return {
				sd: json.result.media.video_sd,
				hd: json.result.media.video_hd,
				title: json.result.info.title
			};
		} else {
			throw new Error('Invalid response structure from yosoyyo-api');
		}
	} catch (error) {
...
async function instagramDl(url) {
	try {
		const dl = await ensureInstagramDownloader();
		if (!dl) throw new Error("instagram-downloader module not available");
